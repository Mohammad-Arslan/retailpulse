...
Last reviewed: 2026-07-28 (Import/export engine hardening Workstreams A–F, a follow-up review pass that found and closed 4 real regressions/gaps in D, C, and F — see IE-D and IE-G — then PR #42 batch 2: counter double-counting, composite locked rules, wizard rule segregation, and insert-only re-import mode — see IE-H through IE-K — then a wizard realtime-progress fix reported from manual QA — see IE-L).
