...
Last reviewed: 2026-07-28 (Import/export engine hardening Workstreams A–F, a follow-up review pass that found and closed 4 real regressions/gaps in D, C, and F — see IE-D and IE-G — then PR #42 batch 2: counter double-counting, composite locked rules, wizard rule segregation, and insert-only re-import mode — see IE-H through IE-K — then a wizard realtime-progress fix reported from manual QA — see IE-L — then a live progress-panel stat-consistency fix — see IE-M).
...
| P11-33 | **Journals could post with `fiscal_year_id = null` when no fiscal year covered the entry date** — `JournalValidationService::assertFiscalYearOpen()` returned silently instead of throwing when `resolveFiscalYear()` found nothing, so a posted entry (including auto-posted sales/purchases/payroll) could land with no fiscal year attached, breaking fiscal-year close and period reporting. Related to the P11-31/P11-32 fiscal-year-resolution family. | **High** | **Resolved 2026-07-26** — the null-fiscal-year branch now throws `App\Exceptions\Accounting\NoOpenFiscalYearException` (a `DomainException`); the null-`fiscal_year_id` date lookup also gained the `Open`/`Reopening`/`Closing` status filter it was missing (previously matched by date only, ignoring status). Enforcement is a hard requirement, not a configurable policy — the phase doc already states posting rules as firm invariants, unlike the backdated-posting policy which is genuinely optional. A new `JournalValidationService::assertFiscalYearOpenForDate()` preflight is called from `CheckoutService::finalizeSale()` (before inventory deduction / FBR submission), `GoodsReceivingService::receive()` (before GRN line processing), and `PayrollRunService::post()` (before the posting transaction), each converting the exception into a friendly, actionable message (`ValidationException` for Checkout/Procurement; the existing generic `DomainException` catch for Payroll) rather than a 500. Fixtures updated to seed a default Open fiscal year: `JournalServiceTest`, `JournalPostingTest`, `AccountingEventServiceTest`, `TaxPostingTest`, `ProcurementWorkflowTest`, `CheckoutFlowTest`, `CheckoutEdgeCasesTest`, `PosCartReservationTest`. |
...
| P12-09 | **No UI to create/manage Salary Structures** — `SalaryStructure`/`SalaryStructureComponent` models and `employees.salary_structure_id` existed, but there was no route, controller, service, policy, or page to create one; only seeding could populate them, blocking payroll setup through the app | **High** | **Resolved 2026-07-26** — `SalaryStructureController` (index/store/update, matching the Designation/Grade no-destroy convention) + `SalaryStructureService` (code generation via `GeneratesMasterCodes`, transactional component-line replace-on-save) + `SalaryStructurePolicy`, gated by the existing `payroll.manage-structures` permission (already seeded in `PermissionSeeder`/`RoleSeeder` for `payroll-officer`, unused until now — no new permission was needed). Routes added under the existing `hr-module:payroll` group. Frontend: `Admin/Payroll/SalaryStructures/Index.jsx`, a modal-driven CRUD page (mirroring `Grades`/`PayComponents`, not a dedicated Create/Edit page — every sibling in this module is modal-based) with an inline pay-component line-item repeater. The employee form's Salary Structure picker (`EmployeeService::createPayload()`/`editPayload()`) needed no changes — it already queried `SalaryStructure` directly and now returns real rows. |
...
| IE-C | **Import resilience & idempotency** — resume checkpoints, dedupe lock, upload guardrails | — | **Resolved 2026-07-26 (Workstream C), corrected 2026-07-26 (review)** — `last_processed_row_index` checkpoint per chunk; `Cache::lock` dedupe prevents double-dispatch while processing; `$tries = 3` with backoff. Config-driven row-count cap, per-tenant concurrent-import limit, rate-limited upload. **Gap found in review:** the per-chunk checkpoint meant `ImportRowSuccess` was only recorded at the end of a fully-completed chunk, while each row's own business write already committed individually — a worker dying mid-chunk (now realistic since `tries = 3` retries automatically) replayed already-succeeded rows on retry: duplicate rows in `create` mode, or a doubled stock adjustment for delta-based handlers (`InventoryAdjustmentImportHandler`) that don't upsert on a natural key. Fixed by writing the `ImportRowSuccess` marker in the *same* transaction as the row's own effects (`ProcessImportJob::processRowWithIsolation`) instead of batching it — see `ProcessImportJobRowIsolationTest::test_resume_after_mid_job_crash_does_not_duplicate_already_succeeded_rows`. Contract documented on `ImportHandler`: a row is processed at most once per job including retries, but only per (job, row index) — handlers that create rather than upsert still need their own natural-key guard against two *different* rows describing the same entity. |
| IE-D | **Import/export realtime auth, throttling, and polling fallback** | — | **Resolved 2026-07-26 (Workstream D), corrected 2026-07-26 (review)** — `import-job.{ulid}` channel now requires both `user_id` AND `tenant_id` match (was tenant-only, a real cross-user privacy hole within a tenant). `stream` renamed to `download-signed`; signed URLs must include `job` ulid and path must belong to that job (owner check); legacy `stream` alias preserved. **Two real regressions found in review, both now fixed:** (1) Workstream D's own polling fallback (`useImportExportJob`, 15s/5s) and `ImportProgressUpdated`'s change to queued `ShouldBroadcast` were themselves the cause of the counter-blink and stale-status bugs QA reported — the polling fallback is removed (Reverb is the single writer to progress state, plus the existing subscribe/reconnect resync), and `ImportProgressUpdated` is back to `ShouldBroadcastNow` to match `ImportCompleted`'s delivery semantics; if Echo is unavailable the UI now shows an explicit "real-time unavailable" state instead of silently freezing. (2) `mergeImportProgress`'s equal-percent-replaces-wholesale branch zeroed validation counters at the validation→processing handoff (both phases cap at 45%) — fixed to only reset counters on a genuine phase-track change, and to clamp (not replace) within a track; neither the hook nor the tray guarded `.progress.updated` against a job that had already reached a terminal status, so a trailing progress broadcast (removed — see below) could flip a completed job back to "processing" in the tray/popup. See `resources/js/lib/importProgress.test.js`, `resources/js/lib/importJobStatus.test.js`, `tests/Feature/ImportExport/ImportEventOrderingTest.php`. |
| IE-E | **Import/export hygiene** — afterImport gate, export tenant audit, registry reset, path docs | — | **Resolved 2026-07-26 (Workstream E)** — `ProcessImportJob` skips `afterImport` when `success_rows === 0` (all-failure import won't trigger batch finalize). Export tenant scoping added to `CustomerExportHandler`, `SupplierExportHandler`, `CoaExportHandler`, `OpeningBalanceExportHandler`, `DepartmentExportHandler`, `SupplierPriceListExportHandler` (were unscoped). `ImportExportRegistry::resetForTesting()` added with note that static registry needs SaaS per-plan rework. Two-import-paths architecture documented in `docs/generic-import-export.md`. |
| IE-F | **Import concurrency locks and safe inventory quantity updates** | — | **Resolved 2026-07-26 (Workstream F), corrected 2026-07-26 (review)** — Advisory `Cache::lock` at top of `ProcessImportJob` keyed on `(entity_type, tenant_id, sha1(input_file_path))` prevents the same uploaded file from running concurrently (complements per-job dedupe lock from C-1; does not serialize all imports of an entity). Audit confirmed `InventoryService` already uses `lockForUpdate()` via `InventoryRepository::lockOrCreate` — no RMW race on `quantity_on_hand`. Workstream A's transient deadlock retry remains intact. **Gap found in review:** `ImportConcurrencyTest` hand-built a lock key that omitted the file-hash component and asserted against its own key, never exercising the job's real lock — it would have passed even with locking removed. The key is now `ProcessImportJob::entityLockKey()`, a single static method the job and its tests both call, and the tests actually drive `handle()` against a pre-held lock (same file blocked; same entity + different file proceeds). Invariant: locked UI rules = UX/drift prevention; DB + row exception handler = correctness floor; multi-worker needs explicit locking for RMW. |
| IE-G | **Partial-success finalization had no direct test** | — | **Resolved 2026-07-26 (review)** — an import ending with row errors hands off to `GenerateErrorReportJob` (not `ProcessImportJob`) to build the error workbook, call `markCompleted()`, and dispatch `ImportCompleted`; existing tests always faked that job away, so nothing proved it actually ran and finalized status rather than leaving the job stuck in `processing`. `PartialSuccessFinalizationTest` runs it for real (dispatched inline on the `sync` queue connection tests use) and asserts the job reaches `completed` with the error report attached. |
| IE-H | **Import summary double-counted rows** — a validation-rejected row was counted in both `failed_rows` and `skipped_rows`, so a 100%-duplicate re-import reported `failed == skipped == total` (impossible: `success+failed+skipped` must equal `total`) | — | **Resolved 2026-07-27 (Batch 2)** — root cause was two places re-stamping the same count onto both buckets: `ValidateImportJob`'s dry-run/strict-abort early return set `failed_rows = skipped_rows = errorCount`, and `GenerateErrorReportJob`'s final update unconditionally did `failed_rows = max(current, errorCount); skipped_rows = max(current, errorCount)` — clobbering whatever `ValidateImportJob`/`ProcessImportJob` had already correctly partitioned. Fixed by establishing one invariant: a row rejected at validation (or never reached because the batch aborted) is `skipped`; `failed` is reserved for rows that reached `processRow` and errored there. `GenerateErrorReportJob` no longer touches the counters at all — it only attaches the report file. `ImportExportJob::markCompleted()` now logs (`Log::error`) if `success+failed+skipped != total` on completion, as a standing regression guard. See `tests/Feature/ImportExport/ImportCounterReconciliationTest.php`. |
| IE-I | **Composite unique constraints were dropped entirely by the schema-derived locked-rules layer** — `SchemaConstraintDeriver` only emitted a locked rule for a *single-column* unique index; any composite index (e.g. inventory's `warehouse_id + product_variant_id + batch_id + bin_location_id`) was silently flagged `inexpressible` with no rule, no pre-flight validation, and no UI signal (`inexpressible_count` wasn't even rendered) | — | **Resolved 2026-07-27 (Batch 2)** — three-part fix: (1) `SchemaConstraintDeriver` now emits a genuine multi-column `unique_in_db` locked rule (anchored on the first mapped column, with `->where()` constraints for the remaining columns) whenever *every* column of a composite unique index maps to a logical CSV field; (2) `UniqueInDbRuleResolver` gained in-file duplicate detection (single- *and* multi-column) via a new `InFileDuplicateTracker`, one instance per `ImportContext` (i.e. per job run) — deliberately *not* stored on the resolver itself, since `RuleResolverRegistry`/its resolvers are bound as singletons for the life of the queue worker and would otherwise leak duplicate state across unrelated jobs; (3) genuinely inexpressible constraints (columns resolved through an FK lookup rather than a direct CSV value, e.g. inventory's warehouse/variant/batch) surface a sanitized, plain-English advisory instead of the opaque count — either handler-supplied (`ImportHandler::compositeConstraintAdvisories()`, since only the handler knows its own lookup semantics) or a generic fallback, never schema identifiers. `InventoryImportHandler::validateRow()` (previously a no-op) now does its own pre-flight duplicate check for the FK-resolved case, reusing `InventoryService::hasExistingOpeningBalance()` (extracted from `setOpeningBalance()`, not re-derived) rather than only discovering the conflict inside `processRow()`. `UniqueInDbRuleResolver` also gained the same empty-table guard `ExistsInDbRuleResolver` already had. See `tests/Unit/ImportExport/UniqueInDbRuleResolverTest.php`, `tests/Feature/ImportExport/InventoryImportCompositeConstraintTest.php`, and the parity/advisory tests in `SchemaLockedConstraintsTest`. |
| IE-J | **Locked vs. custom rule segregation leaked at the "Add rule" step** — `ImportValidationConfig.jsx`'s `addableRules` excluded only already-*enabled* rule types, not *locked* ones, so the Add-rule dropdown still offered e.g. `unique_in_db`/`max` for a field where it was already schema-locked; adding it entered `enabledRules`, got persisted, then vanished from the visible custom-rules list (filtered out by `isEngineLocked`) — "add a rule, it disappears." Separately, the per-field "N rules enabled" count only counted custom rules, so a field with 3 locked rules and 0 custom read as "0 rules enabled" | — | **Resolved 2026-07-27 (Batch 2)** — `addableRules` now also excludes `isEngineLocked` types, so a locked type can never enter `enabledRules` in the first place (clean segregation at every step, not just render); the per-field count is now locked + custom combined; the locked panel renders the composite/inexpressible advisories from IE-I under the chips (`ImportValidationConfig`'s new `CompositeAdvisories`). |
| IE-K | **Opening-stock re-import had no path forward, and batch-1's generic upsert fix doesn't apply to it** — `InventoryImportHandler` is insert-only (`setOpeningBalance()` throws "already exists" by design); re-uploading the same file failed every row with no way to correct a mistaken import, and IE-C's per-row `ImportRowSuccess` marker (which already makes automatic queue retries checkpoint-authoritative — see IE-C) doesn't help a *deliberate* re-import, which is a brand-new job with no shared history | — | **Resolved 2026-07-27 (Batch 2)** — added an explicit opt-in "replace" path: `InventoryService::replaceOpeningBalance()` (shares its DB-write logic with `setOpeningBalance()` via a new private `writeOpeningBalance()` — not a re-derivation) skips the already-exists guard and updates the existing balance instead; `InventoryImportHandler::processRow()`/`validateRow()` branch on `ImportContext::$mode` (`update`/`upsert` ⇒ replace) so this is opt-in only — the default `create` mode is untouched and still fails loudly on a duplicate. The import wizard (`ImportWizardDialog.jsx`) now defaults `inventory` imports to `create` mode (previously defaulted to the generic `upsert` regardless of entity) and shows an explanatory notice ("this import inserts new records only… switch to Update/Upsert to replace"). Deliberately **not** done in this pass: cost layers are not touched by replace mode (`createLayerOnReceive` is skipped) — reconciling unit cost/COGS after replacing an opening balance needs the existing Cost Layer admin tools, not a silent second layer on every re-import. `ImportHandler::isInsertOnly()` (default `false` via `DeclaresImportSchema`) marks the handler for the wizard; `OpeningBalanceImportHandler` (accounting GL opening balances) is also marked `isInsertOnly()` for documentation, but **has no replace mode and the wizard does not yet show a notice for it** — unlike inventory, a re-import doesn't fail (no per-row uniqueness check on `OpeningBalanceImportLine`), it silently appends duplicate staging lines that `finalize()` would post twice into the GL. That's a real, currently-unmitigated double-posting risk, left open here because fixing it needs ledger-reversal semantics (reverse-then-repost), not a row-level replace — tracked here rather than fixed speculatively. `tests/Feature/ImportExport/InventoryImportResumeTest.php` proves the checkpoint-authoritative retry behavior directly against `InventoryImportHandler` (not just the generic test-double handler IE-C used) and the replace-mode end-to-end path. |
| IE-L | **Import wizard could get permanently stuck on the progress step, and closing it while a job ran discarded all progress state** — `useImportExportJob`'s only path to an initial/resync state fetch was `channel.subscribed(() => refresh())`; both `ImportProgressUpdated`/`ImportCompleted` are `ShouldBroadcastNow` (fired synchronously the instant the queue worker finishes), so a small/fast import can validate, process, and broadcast completion *before* the browser's private-channel subscription handshake finishes — most exposed on the very first import of a session, when the WebSocket connection itself is still being established, not just the channel auth. With no live event ever delivered and no fallback, the wizard sat on the progress step forever with no "Done" button, and the `ImportJobsTray`'s per-user channel had the identical exposure. A genuine channel-subscription failure (`pusher:subscription_error`) was never handled either — nothing bound `channel.error(...)`, so a rejected broadcasting-auth request looked identical to "still waiting," with no visible way to recover (the "Refresh Now" affordance in `ImportProgressPanel` only appears when `window.Echo` itself is undefined, not when a specific channel's subscription fails). Separately, closing the wizard while step 5 (progress) was showing reset *all* local state (`ulid`, `step`, `summary`, …) unconditionally, so reopening it started a blank upload form instead of resuming the still-running job — the only way users found to "unstick" the UI (closing and reopening) didn't actually reconnect to anything, it just discarded what little state existed | — | **Resolved 2026-07-28** — `useImportExportJob` now calls `refresh()` once immediately on mount (independent of Echo/WebSocket timing) in addition to the existing `channel.subscribed()` resync, so the UI always gets at least one authoritative REST snapshot regardless of how fast the job finished or how long the socket takes to connect; both the per-job and per-user (`ImportJobsTray`) channels now bind `channel.error(...)` to flag `realtimeUnavailable` (surfacing the existing manual "Refresh Now" button) and trigger one more resync instead of failing silently forever. `ImportWizardDialog`'s close-reset effect now skips resetting state when the dialog closes mid-run (`step === 5`, snapshotted via a ref read at the moment of closing, not reactive to later background step changes) so reopening resumes watching the same job via the hook's continuing subscription; closing before submission or after reaching the done step still resets fully, so the next open starts a clean draft. No new automated test — this repo has no React hook/component test harness (`vitest.config.js` runs plain-JS logic tests only, `environment: 'node'`, no jsdom/Testing Library); adding one is a larger, separate infrastructure decision, not part of this fix. |
| IE-M | **Live progress panel showed impossible stat combinations** — `ImportProgressPanel.jsx` bound its "Processed" card to `combinedProcessed()` (a value deliberately *blended* across the validation and processing phases so the percent bar stays monotonic at the handoff — validation contributes 0–total/2, processing contributes total/2–total) while "Successful"/"Failed" stayed bound to the *raw* phase-local payload values. During validation this meant `Successful` (raw, e.g. 6900) read roughly double `Processed` (blended, 3450); during processing `Successful` (raw, e.g. 1396) read far smaller than `Processed` (blended). Compounding it, the two jobs broadcast `success` on different bases — `ValidateImportJob` sends rows that *passed validation* (`processed - errorCount`), `ProcessImportJob` sends rows *actually imported* (`success_rows`) — so during validation the panel additionally claimed a "Successful" *import* count before anything had been imported at all | — | **Resolved 2026-07-28** — the panel no longer renders a "Processed" stat card at all: `combinedProcessed`/`cumulativeImportProgress` stay exactly as they were (still exported and used) but only for the percent bar and the "X / total records" line, which are explicitly *overall* progress, not a per-phase outcome. The outcome cards (previously always Successful/Failed/Skipped) now bind directly to the raw phase-local payload fields and are labelled by phase via the newly-exported `trackOf()` (from `resources/js/lib/importProgress.js`): **Validated/Invalid** during `validating`/`validated` (nothing has been imported yet, so calling raw-payload rows "Successful" was never accurate), **Successful/Failed/Skipped** during `processing`/`completing` and for terminal statuses. New locale keys `importExport.stats.validated`/`invalid`. Side effect: this required making `.jsx` component modules importable under Vitest at all — `vitest.config.js` previously had no JSX transform (`React is not defined` on any `.jsx` import), so `esbuild: { jsx: 'automatic' }` was added, which is what makes the new component test (`ImportProgressPanel.test.js`, using `react-dom/server`'s `renderToStaticMarkup` — no jsdom/Testing Library needed) possible; this doesn't contradict IE-L's note that no *hook*-level harness exists yet, but component-level rendering tests are now unblocked for future work. |
