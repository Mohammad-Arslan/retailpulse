...
| IE-C | **Import resilience & idempotency** — resume checkpoints, dedupe lock, upload guardrails | — | **Resolved 2026-07-26 (Workstream C)** — `last_processed_row_index` checkpoint per chunk; `Cache::lock` dedupe prevents double-dispatch while processing; `$tries = 3` with backoff for transient job failures; config-driven row-count cap (`import-export.max_row_count`), per-tenant concurrent-import limit (`max_concurrent_imports_per_tenant`), and rate-limited upload route. On re-dispatch after crash, rows ≤ checkpoint are skipped and existing success/error indexes are honoured. |
| IE-D | **Import/export realtime auth, throttling, and polling fallback** | — | **Resolved 2026-07-26 (Workstream D)** — `import-job.{ulid}` channel now requires both `user_id` AND `tenant_id` match (was tenant-only). `ImportProgressUpdated` changed from `ShouldBroadcastNow` to queued `ShouldBroadcast`; ProcessImportJob throttles broadcasts to every 3s. ImportJobsTray no longer full-refreshes on `.progress.updated` (applies counters inline; only refreshes on lifecycle transitions). `useImportExportJob` adds a polling fallback (15s with Echo, 5s without). `stream` route renamed to `download-signed` with owner/tenant ownership check on resolved job; legacy alias preserved. |
...
| IE-F | **Import concurrency locks and safe inventory quantity updates** | — | **Resolved 2026-07-26 (Workstream F)** — Advisory entity-level `Cache::lock` at top of `ProcessImportJob` keyed on `(entity_type, tenant_id)` prevents the same logical import from running concurrently; complements the per-job dedupe lock from C-1. Audit confirmed `InventoryService::applyDelta` and `setOpeningBalance` already use `lockForUpdate()` via `InventoryRepository::lockOrCreate` inside `DB::transaction` — no read-modify-write race on `quantity_on_hand`. Workstream A's transient deadlock retry (`ROW_TRANSIENT_RETRIES = 3`, exponential backoff per row) remains intact and unregressed. Locked UI rules = UX/drift prevention; DB + row exception handler = correctness floor; multi-worker needs explicit locking for RMW. |
