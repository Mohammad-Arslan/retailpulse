...
# These MINIO_*/AWS_* env vars only back the legacy static `s3`/`minio` filesystem disks
# (kept so pre-existing rows uploaded before the change below still resolve). The storage
# backend for NEW uploads — images, attachments, and import/export — is chosen at runtime
# from the admin-configurable Settings → "File Storage" screen (`system_settings`, group
# `file_storage`), not from `.env`. Configure disk type + S3/MinIO/SFTP credentials there
# after first boot.
...
| 2026-07-23 | **Flagged, not fixed**: `composer.lock` pins `symfony/psr-http-message-bridge` to `v8.1.0`, which requires PHP `>=8.4.1` — but this project runs PHP 8.3 everywhere (`composer.json`'s own `"php": "^8.3"`, the Docker image, Laragon locally). The only reason this hasn't broken anything is `Dockerfile`'s `composer install --ignore-platform-reqs`, which silently bypasses the check. CI now does the same (mirroring, not fixing) so it doesn't fail on a pre-existing condition the rest of the project already tolerates. Proper fix — downgrade to a `7.x` version (`laravel/octane` accepts `^2.2.0\|^6.4\|^7.0\|^8.0`) or move the whole project to PHP 8.4 — is a deliberate call for whoever owns dependency upgrades, not something to do as a side effect of making CI pass. |
...
| 2026-07-24 | **Unified file storage**: product/employee images (`MEDIA_DISK` env var), supplier attachments (hardcoded to `local`, no setting at all), and expense attachments (`EXPENSE_ATTACHMENTS_DISK` env var) were three separate, uncoordinated storage mechanisms, independent of the Settings → "Import / export storage" screen — changing that screen to MinIO had zero effect on where images/attachments landed. That screen's settings group (`system_settings`, group `import_export`) is renamed to `file_storage` and now backs all four via a new `App\Services\Storage\FileStorageDiskRegistrar`; each feature keeps its own remote path prefix under the one shared disk/credential set. `MEDIA_DISK` and `EXPENSE_ATTACHMENTS_DISK` env vars are retired — configure the disk in Settings → "File Storage" after first boot instead. Existing files are not migrated; only new uploads follow the new setting (each row's own `disk` column still resolves old files against the unchanged static `s3`/`minio`/`public`/`local` disks). See `docs/architecture/adr-019-shared-file-storage.md`. |
| 2026-07-24 | **Fixed Octane staleness gap**: `ImportExportStorageManager` (and the new `FileStorageDiskRegistrar`) mutate `config('filesystems.disks.*')` from DB-backed settings inside their constructors, but were registered as plain container singletons with `config/octane.php`'s `flush` list empty. Under Octane (confirmed in use — `supervisord.production.conf` runs `octane:start --max-requests=500`), a singleton persists across requests within a worker, so a storage settings change only took effect after that worker's ~500-request recycle, not immediately. Both classes are now in `flush`, so they rebuild fresh (from a 1-hour cached setting read, not a DB hit) on every request. |
