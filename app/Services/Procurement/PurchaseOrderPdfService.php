<?php

declare(strict_types=1);

namespace App\Services\Procurement;

use App\Models\PurchaseOrder;
use App\Models\SystemSetting;
use Barryvdh\DomPDF\Facade\Pdf;
use Illuminate\Support\Facades\Storage;

final class PurchaseOrderPdfService
{
    public function generate(PurchaseOrder $order): string
    {
        $order->load(['supplier', 'branch', 'items.variant.product', 'creator']);

        $pdf = Pdf::loadView('procurement.purchase_order', [
            'order' => $order,
            'company' => [
                'legal_name' => SystemSetting::get('company', 'legal_name', config('app.name')),
                'address' => SystemSetting::get('company', 'address', ''),
                'phone' => SystemSetting::get('company', 'phone', ''),
                'email' => SystemSetting::get('company', 'email', ''),
            ],
            'generatedAt' => now(),
        ]);

        $path = 'procurement/po-'.$order->reference_no.'-'.now()->format('YmdHis').'.pdf';
        Storage::disk('local')->put($path, $pdf->output());

        return $path;
    }
}
