...
export default function Create() {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('admin.customer-groups.store'));
    };

    return (
        <AdminLayout>
            <Head title={t('pages.customerGroups.createTitle')} />
            <PageHeader
                title={t('pages.customerGroups.createTitle')}
                description={t('pages.customerGroups.createDescription')}
            >
            ...
            <form onSubmit={submit} className="max-w-2xl space-y-5">
                ...
                <button type="submit" disabled={processing} className="rp-btn-primary">
                    {t('pages.customerGroups.createSubmit')}
                </button>
            </form>
        </AdminLayout>
    );
