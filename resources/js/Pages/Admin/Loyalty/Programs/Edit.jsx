...
function Edit({ program, branches = [], scopeTypes = [], scopeModes = [] }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        scope_type: program.scope_type ?? 'global',
        earn_scope: program.earn_scope ?? 'global',
        redeem_scope: program.redeem_scope ?? 'global',
        allow_cross_branch_earn: program.allow_cross_branch_earn ?? true,
        allow_cross_branch_redeem: program.allow_cross_branch_redeem ?? true,
        starts_at: program.starts_at ?? '',
        ends_at: program.ends_at ?? '',
        branch_ids: program.branch_ids ?? [],
    });
    ...
    function submit(e) {
        e.preventDefault();
        put(route('admin.loyalty.programs.update', program.id));
    }

    return (
        <>
            <Head title={t('pages.loyalty.programs.editTitle')} />
            <PageHeader title={program.name} description={t('pages.loyalty.programs.editTitle')}>
                <Link href={route('admin.loyalty.programs.show', program.id)} className="rp-btn-outline">
                    {t('confirm.cancel')}
                </Link>
            </PageHeader>
